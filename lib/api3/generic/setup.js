...
function setupGenericCollections (ctx, env, app) {
  ...
  if (_.includes(enabledCols, 'profileSwitch')) {
    cols.profileSwitch = new Collection({
      ctx, env, app,
      colName: 'profileSwitch',
      storageColName: env.profileSwitch_collection || 'profileSwitch'
    });
  }
