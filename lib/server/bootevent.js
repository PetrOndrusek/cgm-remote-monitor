...
function boot (env, language) {
  ...
  function initLog (ctx, next) {

    if (hasBootErrors(ctx)) {
      return next();
    }

    // let's start the logging service ASAP to capture as much events as possible
    ctx.log = require('./log')();
    ctx.log('Booting up Nightscout...');
    next();
  }
  ...
  /**
   * Perform all storage migrations (updates) that are still needed
   * @param ctx
   * @param next
   */
  function migrateStorage (ctx, next) {

    if (hasBootErrors(ctx)) {
      return next();
    }

    require('../storage/migrations')(env, ctx)
      .then(next)
      .catch(function migrationsError (err) {
        console.error('storage migration err', err);
        ctx.bootErrors = ctx.bootErrors || [ ];
        ctx.bootErrors.push({'desc': 'Error during storage migrations', err: err});
        next();
      });
  }
  ...
  function finishBoot (ctx, next) {
    ...
    if (ctx.store && ctx.log) {
      ctx.log.attachToStore(ctx.store, env)
        // let's not slow down the boot process by waiting for database update (.next missing on purpose)
        // only catch errors
        .catch(function attachFailed (err) {
          ctx.log.error(err);
        });
    }
