...
function init (env, cb, forceNewConnection) {
   ...
   */
  mongo.collectionExist = function collectionExist (name) {
    return connection.listCollections({ name: name }).hasNext();
  };
   ...
   */
  mongo.createCappedCollection = function createCappedCollection (name, size) {
    return connection.createCollection(name, {
      capped: true,
      size: size
    });
  };
